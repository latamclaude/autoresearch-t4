...
MAX_SEQ_LEN = 256        # context length (T4 adaptation)
...
if __name__ == "__main__":
    ...
    # Check data exists (must be prepared by Colab notebook Cell 1)
    shards = list_parquet_files()
    if not os.path.exists(DATA_DIR) or not shards:
        print("No data shards found in", DATA_DIR)
        print("Run Cell 1 of the Colab notebook first to download TinyStories data.")
        sys.exit(1)

    print(f"Data: {len(shards)} shards found at {DATA_DIR}")
    print()
